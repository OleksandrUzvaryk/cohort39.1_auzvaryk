    ...
    private void addNewWord() {
        ...
        if (!words.containsKey(key)) {
            ...
            if ("да".equalsIgnoreCase(scanner.nextLine())) {
                words.put(key, value);// если хотим - перезаписываем перевод словаwords.put(key, value);
            }
    ...
    public void addWordToIncorrectSet() {
        ...
        for (String key : words.keySet()) {
            if (key.equalsIgnoreCase(newWordPractice)) {
                System.out.println("Такое слово уже есть в нашей картотеке!");
                break;
            } else {
                incorrectWords.add(newWordPractice);
